...
#define IMGWIDTH 640   // 需传输图像的宽  
#define IMGHEIGHT 480  // 需传输图像的高  
#define PACKAGENUM 2
    //默认格式为CV_8UC3
#define BUFFERSIZE IMGWIDTH*IMGHEIGHT*3/PACKAGENUM
    ...
    public:
        ...
        void setupImgHandle()
        {
            ...
            //调整显示窗口的大小
            windowSize = 500;
            cv::resizeWindow(input, windowSize, windowSize);
            cv::resizeWindow(output, windowSize, windowSize);
            // 鼠标回调函数声明
        ...
        ********************/
        void imageHandle(cv::Mat sImg)
        {
            ...
            {
                //sImg = sImg(cv::Rect(selectRect.x, selectRect.y,
        ...
        ********************/
        ...
        float angleHandle(int pointX)
        {
            float zAngular = 0;
            float zScale = -1.8;
            float centerPoint = windowSize / 2;
        ...
        ********************/
        ...
        int socketConnect(const char *IP, int PORT)
        {
            ...
            while (connect(sockClient, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0) {}
        ...
        int getData()
        {
            ...
            int len0 =  recv(sockClient, rectData, sizeof(rectData), MSG_DONTWAIT);
            if (len0 < 0)
            {
                return -1;
            }
        ...
        */
        ...
        /*
            ...
            ----
        */
        void CTradioClassifier(std::vector<float> &_muPos, std::vector<float> &_sigmaPos, std::vector<float> &_muNeg, std::vector<float> &_sigmaNeg,
