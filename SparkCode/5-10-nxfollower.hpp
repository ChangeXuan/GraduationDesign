    ...
    public:
        ...
        ********************/
        void imageHandle(cv::Mat sImg)
        {
            ...
            if (useWho)
            {
                trackBox = imageCamshift(sImg, selectRect);
            }
            else
            {
            ...
            if (getData() > 0)
            {
                ...
                default:
                    ...
                    if (selectRect.width > 0 && selectRect.height > 0)
                    {
                        ...
                        ctState = 0;
                        //tempImg = sImg(selectRect);
                        cv::cvtColor(sImg(selectRect), tempImg, CV_RGB2GRAY);
                    }
        ...
        ********************/
        cv::Mat testHist;
        cv::Mat testBackproj;
        cv::Rect imageCamshift(cv::Mat sImg, cv::Rect histRect)
        {
            ...
            if (isTrack)
            {
                ...
                cv::Mat testV;
                testV.create(hsvImg.size(), hsvImg.depth());
                ...
                // cv::GaussianBlur(hue,hue,cv::Size(3,3),0);


                // int ch1[] = {2,0};
                // cv::mixChannels(&hsvImg,1,&testV,1,ch1,1);
                ...
                if (isTrack < 0)
                {
                    ...
                    // cv::Mat roi1(testV,histRect),maskRoi1(mask,histRect);
                    // cv::calcHist(&roi1,1,0,maskRoi1,testHist,1,&histSize,&histRange);
                    // cv::normalize(testHist,testHist,0,255,CV_MINMAX);
                ...
                // cv::calcBackProject(&testV,1,0,testHist,testBackproj,&histRange);
                // testBackproj(trackWindow) = ~testBackproj(trackWindow);

                //  backproj &= testBackproj;
                // cv::imshow("addPP",backproj);
                ...
                if (!isMiss)
                {
                    ...
                    // cv::imshow("one",backproj);
                    // cv::threshold(backproj, backproj, 150, 200, CV_THRESH_BINARY);
                    // cv::imshow("two",backproj);
                    // cv::threshold(backproj(shapeRect), backproj(shapeRect), 200, 255, CV_THRESH_BINARY);
                    // cv::imshow("three",backproj);
                    // 腐蚀操作
                    // cv::erode( backproj, backproj, elementE );
                    // // 膨胀操作
                    ...
                    hue = tempImg.clone();
                    cv::imshow("hue",hue);*/
                    cv::CamShift(backproj, trackWindow, cv::TermCriteria( CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 50, 1 ));
                }
                else
                {
                ...
                if (trackWindow.area() <= 5  || trackWindow.x < 5 || abs(trackWindow.x + trackWindow.width - winWidth) < 5)
                {
                    trackWindow = cv::Rect(10, 10, winWidth - 20, winHeight - 20);
                    isMiss = true;
        ...
        /********************
            ...
            up: jia qiang gheng zhon xiao guo
          ********************/
        void imageCamFlower()
        {
        }
        ...
        /********************
            func: 使用pHash算法
          ********************/
        std::string pHashValue(cv::Mat &srcImg, int imgSize)
        {
            cv::Mat img, dst, copyImg;
            int allSize = imgSize * imgSize;
            std::string rst(allSize, '\0');
            double dIndex[allSize];
            double mean = 0.0;
            int k = 0;
            if (srcImg.channels() == 3)
            {
                cv::cvtColor(srcImg, copyImg, CV_RGB2GRAY);
            }
            img = cv::Mat_<double>(copyImg);
            ...
            for (int i = 0; i < imgSize; i++)
            {
                for (int j = 0; j < imgSize; j++)
                {
                    ...
                    mean += dst.at<double>(i, j) / allSize;
                }
            }
        ...
        /********************
            func: jisuan han ming ju
            use: difference<5
          ********************/
        int calHMD(std::string &str1, std::string &str2)
        {
        ...
        // tai kaaaaaa le
        void slideWindow(cv::Mat srcImg)
        {
            cv::Mat image = srcImg.clone();
            cv::Mat sildeImg;
            cv::Mat sildeTemp;
            sildeTemp = image(cv::Rect(selectRect.x, selectRect.y, selectRect.width, selectRect.height));
            int flag = 0;
            for (int i = 0; i < (winWidth - selectRect.width); i++)
            {
                for (int j = 0; j < (winHeight - selectRect.height); j++)
                {
                    sildeImg = image(cv::Rect(i, j, selectRect.width, selectRect.height));
                    std::string str1 = pHashValue(sildeImg, 8);
                    std::string str2 = pHashValue(sildeTemp, 8);
                    if (calHMD(str1, str2) < 5)
                    {
                        //cv::imshow(str1,sildeImg);
                        std::string flagStr = int2Str(flag++);
                        cv::putText(srcImg, flagStr, cv::Point(50, 60), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 23, 0), 4, 8);
                    }
                }
            }
        }
        ...
        /********************
            func: 使用mobanpipei+camshift追踪算法
            up: jia qiang diu shi hou zhao hui
          ********************/
        void imageCamTemp(cv::Mat sImg)
        {
            cv::Mat grayImg;
            cv::Mat matchRes;
            cv::cvtColor(sImg, grayImg, CV_RGB2GRAY);
            // cv::convertScaleAbs(grayImg,grayImg);
            // cv::convertScaleAbs(tempImg,tempImg);
            //模板匹配
            cv::matchTemplate(grayImg, tempImg, matchRes, cv::TM_CCOEFF_NORMED);
            ...
            if (maxVal > 0.80)
            {
                cv::rectangle(sImg, cv::Rect(maxLoc.x, maxLoc.y, selectRect.width, selectRect.height), cv::Scalar(0, 0, 255), 4);
                trackWindow = cv::Rect(maxLoc.x, maxLoc.y, selectRect.width, selectRect.height);
                isMiss = false;
            }
